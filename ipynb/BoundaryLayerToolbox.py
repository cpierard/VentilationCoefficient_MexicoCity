...
def wrf2dataframe(var_24, var_48, month_t_range, ix, iy, var_str):
    """
        wrf2dataframe(var_24, var_48, month_t_range, ix, iy):
        - var_24 : salida de variable 24 para el dominio.
        - var_48 : salida de variable 48 para el dominio.
    ...
    month_var_24 = []
    month_var_48 = []
    for i in range(0,len(var_24[:,0,ix,iy])):
        for j in range(0,24):
            month_var_24.append(var_24[i,j,ix,iy])
            month_var_48.append(var_48[i,j,ix,iy])

    month_wrf_df = pd.DataFrame({var_str + '_24' : pd.Series(month_var_24, index=month_t_range),
                               var_str + '_48' : pd.Series(month_var_48, index=month_t_range)})
    return month_wrf_df
