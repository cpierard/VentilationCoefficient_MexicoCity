...
def interpolate_velocity(XLONG, XLAT, XLONG_U, XLAT_U, U, uu=True,
                         Kind='linear'):
    '''
    `XLON`, `XLAT`, `XLONG_U`, `XLAT_U, U` van con su dimensión temporal.
    `U` va con su dimensión temporal.
    `uu` define si se trata de velocidad zonal o meridional, es decir si se
    está interpolando 'u', uu = True.
...
def compute_height(PH, PHB):
    '''
    PH y PHB deben ser prporcionados como una matriz de 4 dimensiones es decir
    no hay especificar su variable temporal.
    '''
    g = 9.8
    ...
    for t in range(0, PH.shape[0]):
        for h in range(0, PH.shape[1]-1):
            Z[t, h, :, :] = ((PH[t, h, :, :] + PHB[t, h, :, :]) +
                             (PH[t, h+1, :, :] + PHB[t, h+1, :, :]))/(g*2)
...
def compute_Richardson(T_s, θ, Z, u, v):
    ...
    θ = Temperatura potencial. Tiene que ser un arreglo 1d con una temperatura
    potencial por cada nivel.
    ...
    Ri = un arreglo con las mismas dimensiones que Z, con los Richardson
    calculados para cada altura.
    OUT: Ri
    '''
...
def near_coord_loc(xlong, xlat, loclong, loclat):
    ...
    Busca los índices más cercanos a una coordenada. Para matrices normales
    (verificar indices de todas formas).
    '''
...
def detecta_PBL_Ri(Ri, Z, Ric):
    ...
    Decta el índice de la primera capa donde el gradiente es igual al valor
    crítico. n es el índice y z es la altura donde está la capa límite.
...
def detecta_PBL_indices(Ri, Z, Ric):
    ...
    Decta el conjunto de índices donde el gradiente es igual al valor crítico.
    n es un arreglo que contiene todos los índices.
    Ri = perfil vertical de Richardon.
    Z = alturas para una coordenada
    Ric = valor crítico de Richardson. Usualmente es 0.21.
    '''
...
nombres_delegaciones = ["alvaro_obregon",
                        "azcapotzalco",
                        "benito_juarez",
                        "coyoacan",
                        "cuajimalpa",
...
class region:
    """
    Declares a new class specific for geografical polygons. The main purpose is
    to locate which points in a grid are inside of the polygon.
    ATRIBUTES:
        self.lon: Is an array with all the longitudinal coordinates of the
        polygon.
...
def read_region(region, f):
    ...
    The function reads an .txt file and fills up its attributes, except
    region.nx and region.ny.
    'region' is an object class region, previously declared as region().
    f is the file location, it must be a string.
...
def in_or_out(region, x, y):
    ...
    Checks if the point x and y, is inside of the polygon region. The polygon
    must be an object class region.
    OUT: boolean.
    """
    crossings = 0
...
def points_in_region(region, x_long, x_lat):
    ...
    Looks for the points inside a polygon. 'region' is the polygon it must be
    an object region.
    ...
    OUT: nothing. It appends the nx and ny indices of the points inside of the
    polygon to the atribitute region.nx and region.ny .
    """
    nx = []
    ny = []
    for i in range(0, x_long.shape[0]):
        for j in range(0, x_long.shape[1]):
            if in_or_out(region, x_long[i, j], x_lat[i, j]):
                nx.append(i)
                ny.append(j)
...
def import_var_mat(file, station):
    ...
    Imports the variables in a .mat file previously read using
    scipy.io.loadmat() function.
    The argument station is the name or acronym of the station, it must be a
    string.
    It returns a directory containing all the arrays.
    """
    variables = ["PBLH", "T", "T2", "U", "V",
                 "PH", "PHB", "HGT", "XLAT", "XLONG"]
    d = {}
    for i in variables:
...
def read_ceilometro_month(file_path):
    ...
    Is a function that imports data of the ceilometer data per month.
    You can find the data in the folder Datos/ceilometro/215_utc.
    """
    datetimes = []
    raws = []
    filtereds = []
...
def promedios_mesuales_hora(hh, DT, RAW):
    ...
    R = (mean_mesuales, std_mensuales, mean_mensuales_hora, std_mensuales_hora)
    return R
...
def promedios_mensuales(DT, RAW):
    ...
    mes_inteval_std = []
    for h in range(0, 24):
        unpack = promedios_mesuales_hora(h, DT, RAW)
        mean_mm, std_mm, mean_mm_intv, std_mm_intv = unpack
        mes.append(mean_mm)
...
def wrf2dataframe(vc_24, pblh_24, u_24, month_t_range, ix, iy):
    ...
    - ix y iy son los índices que corresponden al punto más cercano a una
    estación en el dominio.
    """
...
def U_mean_PBL(U, V, PBLH, HGT, Z):
    ...
    Computes the mean velocity magnitud under de PBL for a 2D domain with
    temporal dimension.
    """
    # Creates the new array to store the VC for each point and time.
    U_mean_PBL = np.zeros_like(PBLH)
    # Computes the magnitud of the velocity
    U_mean = np.sqrt(U**2 + V**2)
    # Computes de height indexes that correspond to the PBLH.
    ix_pblh = find_pblh_index(PBLH, HGT, Z)
        ...
        for i in range(0, Z.shape[2]):
            for j in range(0, Z.shape[3]):
                ...
                if ix_pblh[t, i, j] == 0:  # to avoid nan in the data.
                    ...
                    # the average of the velocity magnitude in the mixed layer.
                    a = ix_pblh[t, i, j]
                    vertical_U_mean = np.mean(U_mean[t, :a, i, j])
...
def U_PBL(U, PBLH, HGT, Z):
    ...
    Computes the ventilation coefficient for a 2D domain with temporal
    dimension
    """

    U_PBL = np.zeros_like(PBLH)
    # Creates the new array to store the VC for each point and time.
    ix_pblh = find_pblh_index(PBLH, HGT, Z)
    # Computes de height indexes that correspond to the PBLH.
        ...
        for i in range(0, Z.shape[2]):
            for j in range(0, Z.shape[3]):
                ...
                if ix_pblh[t, i, j] == 0:  # to avoid nan in the data.
                    ...
                    vertical_U_mean = np.mean(U[t, :ix_pblh[t, i, j], i, j])
                    # the average of the velocity magnitude in the mixed layer.
    ...
    for t in range(0, Z.shape[0]):
        for i in range(0, Z.shape[2]):
            for j in range(0, Z.shape[3]):
                ...
                vc[t, i, j] = PBLH[t, i, j] * vertical_U_mean
                # computing the VC.
...
def VCS(dir_month):
    """
    VCS(dir_month):
        Calcula el producto punto de la velocidad con el gradiente del relieve,
        para un directorio con las siguientes variables:
        'U', 'V', 'HGT',
...
def interpolate_field(x_range, y_range, XLONG_field, XLAT_field, field,
                      Kind='linear'):
    '''
    interpolate_field(x_range, y_range, XLONG_field, XLAT_field, field,
    Kind='linear')

    Hola. Aquí debe haber una descripción de la función, pero no la hay...
    '''
    nx = len(x_range)
    ny = len(y_range)
    ...
    if len(field.shape) == 4:
        ...
        for t in range(0, field.shape[0]):
            for h in range(0, field.shape[1]):
                ...
                f = interpolate.interp2d(x_old, y_old, field[t, h, :, :],
                                         kind=Kind)
    ...
    for t in range(0, t_len):
        for x in range(0, x_len):
            for y in range(0, y_len):
                NN[t, :, x, y] = g/T_pot[t, :h_len-1, x, y] * \
                    np.diff(T_pot[t, :, x, y])/np.diff(Z[t, :, x, y])
    return NN
