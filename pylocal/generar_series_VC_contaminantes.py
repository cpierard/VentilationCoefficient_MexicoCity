...
for st in location:
    ...
    #contaminantes
    o3_2015 = pd.read_csv(path2pollutants + st + "/" + st + "_O3_2015.csv", names = ['date', 'station', 'pollutant', 'O3', 'units'])
    ...
    pm25_2015 = pd.read_csv(path2pollutants + st + "/" + st + "_PM2.5_2015.csv", names = ['date', 'station', 'pollutant', 'PM2.5', 'units'])
    pm25_2015.index = pd.date_range('2015-01-01 01:00', '2016-01-01 00:00', freq='1H')
    pm25_2015 = pm25_2015.reindex(pd.date_range('2015-01-01 00:00', '2015-12-31 23:00', freq='1H'))
    pm25_2015 = pm25_2015.drop(['date', 'station', 'pollutant', 'units'], axis = 1)

    pm10_2015 = pd.read_csv(path2pollutants + st + "/" + st + "_PM10_2015.csv", names = ['date', 'station', 'pollutant', 'PM10', 'units'])
    pm10_2015.index = pd.date_range('2015-01-01 01:00', '2016-01-01 00:00', freq='1H')
    pm10_2015 = pm10_2015.reindex(pd.date_range('2015-01-01 00:00', '2015-12-31 23:00', freq='1H'))
    pm10_2015 = pm10_2015.drop(['date', 'station', 'pollutant', 'units'], axis = 1)
    ...
    for mm in months.keys():
        ...
        if mm == 'nov':
            ...
            BEG = pd.date_range('2015-10-01 00:00:00', '2015-10-03 23:00:00', freq='1H')
            ENDD = pd.date_range('2015-10-05 00:00:00', '2015-10-31 23:00:00', freq='1H')
            month_t_range = BEG.union(ENDD)

        elif mm == 'jan':
            month_t_range = pd.date_range('2015-01-15 00:00:00', '2015-01-31 23:00:00', freq='1H') ##Falta esta fechas
        ...
        wrf_df = blt.wrf2dataframe(vc_24, pblh_24, u_mean_24, month_t_range, xx, yy)
    ...
    ALL.to_csv(path2DataFrames + st + '/vc_'+ st +'.csv', float_format='%.2f')#, na_rep='nan')
    print(st + ' -- SAVED')
