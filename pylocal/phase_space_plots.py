...
xi = np.linspace(100, 3800, nx)
yi = np.linspace(1, 14.5, ny)
...
ax3.grid(linestyle='--')
CS = ax3.contour(xxx, yyy, VC_grid, levels=[6e3, 8e3, 10e3, 12e3, 14e3],
                 colors='k')
ax3.clabel(CS, fontsize=13, inline=1, fmt='%1.0f')
...
ax3.scatter(day_ALL.loc[day_ALL['PM10'] < nom_pm10]['pblh'],
            ...
            c=color_2, alpha=0.7, label=r"PM10 < 75 $\mu g \ m^{-3}$", s=10)
ax3.scatter(day_ALL.loc[day_ALL['PM10'] > nom_pm10]['pblh'],
            day_ALL.loc[day_ALL['PM10'] > nom_pm10]['u_mean'],
            c=color_1, alpha=0.7, label=r"PM10 > 75 $\mu g \ m^{-3}$", s=10)
ax3.set_xlabel(r'PBLH ($m$)', fontsize=13)
ax3.set_ylabel(r'Mean wind velocity ($m \ s^{-1}$)', fontsize=13)
ax3.set_xlim(0, 4000)
ax3.set_ylim(0, 15)
